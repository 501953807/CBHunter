...
def _market_note(recommendation: dict[str, Any]) -> str:
    ...
    if present_labels:
        return f"{scope} 已有{'、'.join(present_labels)}证据，可进入精细化验证。"
    return f"{scope} 仍缺少核心证据，先补趋势、平台、供应和竞品数据。"
...
def _pricing_note(recommendation: dict[str, Any]) -> str:
    ...
    return f"暂无竞品均价，1688 采购参考 {sourcing_price}，定价前需补平台价格证据。"
...
def _risk_note(recommendation: dict[str, Any]) -> str:
    ...
    if gaps:
        return f"证据完整度 {present}/{total}，当前待补：{'、'.join(gaps)}。"
    return f"证据完整度 {present}/{total}，仍需在刊登前复核缺失或低置信字段。"
