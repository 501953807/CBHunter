...
export function matchesHighlightedField(field: PlatformField, highlightedFieldKey: string) {
...
export function buildCategoryProfileSummary(requirements: PlatformRequirementsLike | undefined, groups: PlatformFieldGroup[], fields: PlatformField[]) {
...
export function CategoryProfileRuntimeSummary({ summary }: { summary: ReturnType<typeof buildCategoryProfileSummary> }) {
...
export function FieldReadinessStrip({ total, filled, missing, recheckCount }: { total: number; filled: number; missing: string[]; recheckCount: number }) {
...
export function FieldFocusToolbar({
  search,
  onSearch,
  filter,
  onFilter,
...
export function FieldEvidenceAuditSummary({ fields, requirements }: { fields: PlatformField[]; requirements?: PlatformRequirementsLike }) {
...
export function FieldMetaHint({ field }: { field: PlatformField }) {
  const details = [
...
export function fieldMatchesFocus(field: PlatformField, values: Record<string, unknown>, search: string, filter: 'all' | 'missing' | 'recheck') {
...
export function fallbackAttrMatchesFocus(attr: string, values: Record<string, unknown>, search: string, filter: 'all' | 'missing' | 'recheck') {
  const query = search.trim().toLowerCase()
...
export function groupFieldStats(fields: PlatformField[], values: Record<string, unknown>) {
...
export function FieldRequirementHint({ field, hasValue }: { field: PlatformField; hasValue: boolean }) {
...
export function FieldValueControl({ field, value, onChange }: { field: PlatformField; value: string; onChange: (value: string) => void }) {
...
export function FieldLine({ field, value }: { field: PlatformField; value: unknown }) {
...
export function FallbackAttributeList({ attrs, values }: { attrs: string[]; values: Record<string, unknown> }) {
  return (
...
export function normalizeGroups(requirements?: PlatformRequirementsLike): PlatformFieldGroup[] {
...
export function hasFieldValue(field: PlatformField, values: Record<string, unknown>) {
  return Boolean(stringValue(values[field.key || field.label || '']).trim())
}

export function evidenceStateLabel(state: PlatformField['evidence_state']) {
